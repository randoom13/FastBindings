...
{
    public class AsyncOneWayFastBinding : BaseOneWayBinding
    {
        public AsyncOneWayFastBinding()
        {
        }

        public AsyncOneWayFastBinding(string target)
        {
            Target = target;
        }

        private readonly AsyncOneWayBindingUpdateManager<IPropertyAccessor> _updateManager =
    new AsyncOneWayBindingUpdateManager<IPropertyAccessor>(new FastViewModelTreeHelper());
        ...
        {
            VisualTreeHelperEx.SubscribeLoaded(element, OnTargetLoaded);
        }

        private void OnTargetLoaded(object sender, EventArgs args)
        {
            var element = sender as DependencyObject;
            if (VisualTreeHelperEx.UnSubscribeLoaded(element, OnTargetLoaded))
            {
