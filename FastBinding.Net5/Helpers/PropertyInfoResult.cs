...
namespace FastBindings.Helpers
{
    internal class PropertyInfoResult
    {
        ...
        public bool Set<T>(object target, object? value)
        {
            ...
            if (!HasSetter)
            {
                System.Diagnostics.Debug.Write($"setter for {_name} {typeof(T)} is absent");
                return false;
            }
            ...
            {
                System.Diagnostics.Debug.Write($"target [{_name}] should be {typeof(T)} but in reality is {target?.GetType()?.ToString() ?? "None"}");
                return false;
            }

            if (value?.GetType() != PropertyType)
            {
                System.Diagnostics.Debug.Write($"setter for {_name} {PropertyType?.ToString() ?? "none"} but value is {value?.GetType()?.ToString() ?? "None"}");
        ...
        public object? Get<T>(object target)
        {
            if (!IsValid)
            {
                System.Diagnostics.Debug.Write($"Failed to create getter for {typeof(T)}");
                return false;
            }

            if (!HasGetter)
            {
                System.Diagnostics.Debug.Write($"getter for {_name} {typeof(T)} is absent");
                return null;
            }
        ...
        private Func<T, object?>? CreateGetter<T>(object target)
        {
            ...
            {
                System.Diagnostics.Debug.Write($"Failed to create expression tree for getter");
        ...
        private Action<T, object?>? CreateSetter<T>(object target)
        {
            ...
            {
                System.Diagnostics.Debug.Write($"Failed to create expression tree for setter");
                System.Diagnostics.Debug.Write(ex);
                System.Diagnostics.Debug.Write(ex.StackTrace);
                return null;
            }
